...
function isGenericSessionName(name: string | undefined, sessionId: string): boolean {
  const trimmed = name?.trim();
  if (!trimmed) return true;
  if (trimmed.startsWith('Bridge:')) return true;
  if (trimmed === sessionId) return true;
  if (trimmed === sessionId.slice(0, 8)) return true;
  if (trimmed === `Session ${sessionId.slice(0, 8)}`) return true;
  return false;
}

function summarizeSessionName(text: string, maxLength = 28): string {
  const cleaned = text
    .replace(/<!--files:[\s\S]*?-->/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
export async function processMessage(
  ...
  try {
    ...
    if (session && isGenericSessionName(session.name, sessionId)) {
      const suggestedName = summarizeSessionName(text);
      if (suggestedName) {
        try { store.updateSessionName(sessionId, suggestedName); } catch { /* best effort */ }
      }
    }
