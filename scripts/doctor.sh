...
if [ -f "$CONFIG_FILE" ]; then
  if is_windows_like; then
    check "config.env permissions check (skipped on Windows-managed filesystems)" 0
  else
...
if [ -f "$PID_FILE" ]; then
  PID=$(cat "$PID_FILE")
  if is_windows_like; then
    if powershell.exe -NoProfile -Command "exit [int](-not (Get-Process -Id $PID -ErrorAction SilentlyContinue))" >/dev/null 2>&1; then
...
if [ -f "$LOG_FILE" ]; then
  ERROR_COUNT=$(tail -50 "$LOG_FILE" | grep -viE 'DEP0169|DeprecationWarning' | grep -ciE 'ERROR|Fatal' || true)
