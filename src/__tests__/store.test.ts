...
describe('JsonFileStore', () => {
  ...
  it('createSession and getSession', () => {
    ...
    assert.equal((session as { name?: string }).name, 'test');
  ...
  it('updateSessionName persists renamed sessions', () => {
    const store = new JsonFileStore(makeSettings());
    const session = store.createSession('Bridge: chat-1', 'model-1', undefined, '/tmp/one');

    store.updateSessionName(session.id, 'Investigate payment timeout');

    const updated = store.getSession(session.id) as { name?: string };
    assert.equal(updated.name, 'Investigate payment timeout');
  });

  it('archiveSession hides sessions from the active list', () => {
    const store = new JsonFileStore(makeSettings());
    const active = store.createSession('active', 'model-1', undefined, '/tmp/one');
    const archived = store.createSession('archived', 'model-1', undefined, '/tmp/two');
    ...
    assert.ok((store.getSession(archived.id) as { archived_at?: string }).archived_at);
  });
