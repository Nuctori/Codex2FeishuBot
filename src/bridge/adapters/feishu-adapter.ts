...
interface FeishuStreamingCardContext {
  projectLabel: string;
  projectPath: string;
  sessionLabel: string;
}

interface FeishuTypingReactionState {
  requestId: string;
  messageId: string;
  reactionId: string;
}
...
function getProjectLabelFromPath(projectPath: string): string {
  ...
  if (!normalized) return localizeText('Project', '项目');
...
function normalizeFeishuDomainSetting(domainSetting: string | null | undefined): 'feishu' | 'lark' {
  const normalized = String(domainSetting || '').trim().toLowerCase();
  if (normalized === 'lark' || normalized.includes('larksuite')) {
    return 'lark';
  }
  return 'feishu';
}
...
export class FeishuAdapter extends BaseChannelAdapter {
  ...
  private seenCardActionTokens = new Map<string, boolean>();
  ...
  /** Track active typing reactions per request for cleanup. */
  ...
  async start(): Promise<void> {
    ...
    const domainSetting = normalizeFeishuDomainSetting(getBridgeContext().store.getSetting('bridge_feishu_domain'));
  ...
  async stop(): Promise<void> {
    ...
    this.seenCardActionTokens.clear();
   ...
   */
  private getStreamKey(chatId: string, context?: StreamContext): string {
    return context?.requestId?.trim() ? `${chatId}:${context.requestId}` : chatId;
  }

  onMessageStart(chatId: string, context?: StreamContext): void {
    const messageId = context?.messageId || this.lastIncomingMessageId.get(chatId);
    const streamKey = this.getStreamKey(chatId, context);

    // Create streaming card (fire-and-forget — fallback to traditional if fails)
    if (messageId) {
      this.createStreamingCard(chatId, context, messageId).catch(() => {});
    }
    ...
    this.restClient.im.messageReaction.create({
      ...
      if (reactionId) {
        this.typingReactions.set(streamKey, {
          requestId: context?.requestId || chatId,
          messageId,
          reactionId,
        });
      }
   ...
   */
  onMessageEnd(chatId: string, context?: StreamContext): void {
    const streamKey = this.getStreamKey(chatId, context);
    // Clean up any orphaned card state (normally cleaned by finalizeCard)
    this.cleanupCard(chatId, context);

    // Remove typing reaction (same as before)
    const reaction = this.typingReactions.get(streamKey);
    if (!reaction || !this.restClient) return;
    this.typingReactions.delete(streamKey);
  ...
  private async handleCardAction(data: unknown): Promise<unknown> {
    ...
    try {
      ...
      let callbackData = value.callback_data;
      if (!callbackData) {
        if (value.nav === 'projects') {
          callbackData = `nav:projects${value.scope ? `:${value.scope}` : ''}`;
        } else if (value.nav === 'workspace' && value.workspace_path) {
          const encodedWorkspace = Buffer.from(String(value.workspace_path), 'utf8').toString('base64url');
          callbackData = `nav:workspace:${encodedWorkspace}`;
        } else if (value.nav === 'status') {
          callbackData = `nav:status${value.page != null ? `:${value.page}` : ''}`;
        } else if (value.nav === 'ctx_status') {
          callbackData = `nav:ctx:status:${value.page ?? 0}:${value.index ?? 0}`;
        } else if (value.nav === 'ctxclear_status') {
          callbackData = `nav:ctxclear:status:${value.page ?? 0}`;
        } else if (value.nav === 'project' && value.project_path) {
          const encodedPath = Buffer.from(String(value.project_path), 'utf8').toString('base64url');
          const encodedWorkspace = value.workspace_path
            ? Buffer.from(String(value.workspace_path), 'utf8').toString('base64url')
            : null;
          callbackData = encodedWorkspace
            ? `nav:project:${encodedPath}:${encodedWorkspace}${value.page != null ? `:${value.page}` : ''}`
            : `nav:project:${value.scope === 'global' ? 'global' : 'workspace'}:${encodedPath}${value.page != null ? `:${value.page}` : ''}`;
        } else if (value.nav === 'project_new' && value.project_path) {
          const encodedPath = Buffer.from(String(value.project_path), 'utf8').toString('base64url');
          const encodedWorkspace = value.workspace_path
            ? Buffer.from(String(value.workspace_path), 'utf8').toString('base64url')
            : null;
          callbackData = encodedWorkspace
            ? `nav:new:${encodedPath}:${encodedWorkspace}`
            : `nav:new:${value.scope === 'global' ? 'global' : 'workspace'}:${encodedPath}`;
        } else if (value.nav === 'bind' && value.session_id) {
          callbackData = `nav:bind:${value.session_id}`;
        } else if (value.nav === 'dock_select' && value.session_id) {
          callbackData = `nav:dock:select:${value.session_id}`;
        } else if (value.nav === 'dock_close' && value.session_id) {
          callbackData = `nav:dock:close:${value.session_id}`;
        } else if (value.nav === 'peek' && value.session_id) {
          callbackData = `nav:peek:${value.session_id}${value.page != null ? `:${value.page}` : ''}`;
        } else if (value.nav === 'ctx_peek' && value.session_id) {
          callbackData = `nav:ctx:peek:${value.session_id}:${value.page ?? 0}:${value.index ?? 0}`;
        } else if (value.nav === 'ctxclear_peek' && value.session_id) {
          callbackData = `nav:ctxclear:peek:${value.session_id}:${value.page ?? 0}`;
        } else if (value.nav === 'archive' && value.session_id) {
          callbackData = `nav:archive:${value.session_id}`;
        }
      }
      ...
      if (callbackToken && this.seenCardActionTokens.has(callbackToken)) {
        ...
        return { toast: { type: 'info' as const, content: '姝ｅ湪澶勭悊涓?..' } };
      }
      if (callbackToken) {
        this.addCardActionToDedup(callbackToken);
      }
      ...
      const callbackMsg: import('../types.js').InboundMessage = {
        messageId: callbackToken || messageId || `card_action_${Date.now()}`,
        address: {
          channelType: 'feishu',
          chatId,
          userId,
   ...
   */
  private createStreamingCard(chatId: string, context?: StreamContext, replyToMessageId?: string): Promise<boolean> {
    const streamKey = this.getStreamKey(chatId, context);
    if (!this.restClient || this.activeCards.has(streamKey)) return Promise.resolve(false);

    // In-flight guard: if creation is already in progress, return the existing promise
    const existing = this.cardCreatePromises.get(streamKey);
    if (existing) return existing;

    const promise = this._doCreateStreamingCard(chatId, context, replyToMessageId);
    this.cardCreatePromises.set(streamKey, promise);
    promise.finally(() => this.cardCreatePromises.delete(streamKey));
    return promise;
  }

  private async _doCreateStreamingCard(chatId: string, context?: StreamContext, replyToMessageId?: string): Promise<boolean> {
    if (!this.restClient) return false;
    const streamKey = this.getStreamKey(chatId, context);

    try {
      const navContext = this.getStreamingCardContext(chatId, context);
      const cardBody = {
        schema: '2.0',
        config: {
          streaming_mode: true,
          wide_screen_mode: true,
          summary: { content: localizeText('Thinking...', '正在思考...') },
        },
        body: {
          elements: [
            ...(navContext ? [
            ...
            {
              tag: 'markdown',
              content: localizeText('Thinking...', '正在思考...'),
              text_align: 'left',
              text_size: 'normal',
      ...
      // Store card state
      this.activeCards.set(streamKey, {
        streamKey,
        chatId,
        cardId,
        messageId,
        ...
        context: navContext || {
          ...
          sessionLabel: context?.sessionId ? `\`${getShortSessionId(context.sessionId)}\`` : '',
        },
      });
   ...
   */
  private updateCardContent(chatId: string, text: string, context?: StreamContext): void {
      ...
      // Schedule trailing-edge flush
        if (!state.throttleTimer) {
          state.throttleTimer = setTimeout(() => {
            ...
            this.flushCardUpdate(chatId, context);
   ...
   */
  private flushCardUpdate(chatId: string, context?: StreamContext): void {
    const state = this.activeCards.get(this.getStreamKey(chatId, context));
    if (!state || !this.restClient) return;
  ...
  private async finalizeCard(
    ...
    const pending = this.cardCreatePromises.get(streamKey);
    if (pending) {
    ...
    try {
      ...
      console.warn('[feishu-adapter] Card settings finalize failed:', err instanceof Error ? err.message : err);
    }

    try {
      // Step 2: Build and apply final card
      const statusLabels: Record<string, string> = {
        completed: localizeText('\u2705 Completed', '\u2705 \u5DF2\u5B8C\u6210'),
        interrupted: localizeText('\u26A0\uFE0F Interrupted', '\u26A0\uFE0F \u5DF2\u4E2D\u65AD'),
        error: localizeText('\u274C Error', '\u274C \u51FA\u9519'),
      };
      ...
      if (streamingClosed) {
        try {
          ...
          // Ignore fallback update failures; caller will send a normal response.
        }
      }
      return false;
    } finally {
   ...
   */
  private cleanupCard(chatId: string, context?: StreamContext): void {
    const streamKey = this.getStreamKey(chatId, context);
    this.cardCreatePromises.delete(streamKey);
    const state = this.activeCards.get(streamKey);
    if (!state) return;
    if (state.throttleTimer) {
      clearTimeout(state.throttleTimer);
   ...
   */
  hasActiveCard(chatId: string, context?: StreamContext): boolean {
    return this.activeCards.has(this.getStreamKey(chatId, context));
  }

  private getStreamingCardContext(chatId: string, context?: StreamContext): FeishuStreamingCardContext | null {
    const projectPathFromContext = context?.workingDirectory?.trim();
    if (projectPathFromContext) {
      return {
        projectLabel: getProjectLabelFromPath(projectPathFromContext),
        projectPath: projectPathFromContext,
        sessionLabel: context?.sessionId ? `\`${getShortSessionId(context.sessionId)}\`` : '',
      };
    }

    const { store } = getBridgeContext();
    const binding = store.getChannelBinding('feishu', chatId);
    if (!binding) return null;
    const projectPath = binding.workingDirectory || '';
    const session = typeof store.getSession === 'function'
      ? store.getSession(binding.codepilotSessionId)
      : null;
    const sessionLabel = session?.name
      ? `${session.name} · \`${getShortSessionId(binding.codepilotSessionId)}\``
      : `\`${getShortSessionId(binding.codepilotSessionId)}\``;
    return {
      projectLabel: getProjectLabelFromPath(projectPath),
      projectPath,
      sessionLabel,
    };
  }
   ...
   */
  onStreamText(chatId: string, fullText: string, context?: StreamContext): void {
    if (!this.activeCards.has(this.getStreamKey(chatId, context))) {
      // Card should have been created by onMessageStart, but create lazily if not
      const messageId = context?.messageId || this.lastIncomingMessageId.get(chatId);
      this.createStreamingCard(chatId, context, messageId).then((ok) => {
        if (ok) this.updateCardContent(chatId, fullText, context);
      }).catch(() => {});
      return;
    }
    this.updateCardContent(chatId, fullText, context);
  }

  onToolEvent(chatId: string, tools: ToolCallInfo[], context?: StreamContext): void {
    this.updateToolProgress(chatId, tools, context);
  }

  async onStreamEnd(chatId: string, status: 'completed' | 'interrupted' | 'error', responseText: string, context?: StreamContext): Promise<boolean> {
    return this.finalizeCard(chatId, status, responseText, context);
  }
  ...
  async send(message: OutboundMessage): Promise<SendResult> {
    if (!this.restClient) {
      return { ok: false, error: localizeText('Feishu client not initialized', '飞书客户端未初始化') };
    }

    if (message.parseMode === 'CardJson') {
      return this.sendCardJson(message.address.chatId, message.text, message.updateMessageId);
    }
  ...
  private async sendCardJson(chatId: string, cardJson: string, updateMessageId?: string): Promise<SendResult> {
    try {
      if (updateMessageId) {
        const patchRes = await this.restClient!.im.message.patch({
          path: { message_id: updateMessageId },
          data: {
            content: cardJson,
          },
        });
        const patchCode = typeof patchRes?.code === 'number' ? patchRes.code : undefined;
        if (patchCode === undefined || patchCode === 0) {
          console.log('[feishu-adapter] Card updated in place:', JSON.stringify({
            messageId: updateMessageId,
            code: patchCode ?? null,
          }));
          return { ok: true, messageId: updateMessageId };
        }
        console.warn('[feishu-adapter] Card update failed:', JSON.stringify({
          messageId: updateMessageId,
          code: patchCode,
          msg: patchRes?.msg ?? null,
        }));
        return {
          ok: false,
          error: patchRes?.msg || localizeText(`Card update failed (${patchCode})`, `卡片更新失败（${patchCode}）`),
        };
      }
      ...
      return { ok: false, error: res?.msg || localizeText('Card send failed', '卡片发送失败') };
    } catch (err) {
      if (updateMessageId) {
        console.warn('[feishu-adapter] Card update error:', err instanceof Error ? err.message : err);
      }
      return { ok: false, error: err instanceof Error ? err.message : localizeText('Card send failed', '卡片发送失败') };
    }
  }
  ...
  private async sendAsPost(chatId: string, text: string): Promise<SendResult> {
    ...
    try {
      ...
      return { ok: false, error: res?.msg || localizeText('Send failed', '发送失败') };
    } catch (err) {
      return { ok: false, error: err instanceof Error ? err.message : localizeText('Send failed', '发送失败') };
    }
  }
  ...
  private async sendPermissionCard(
    ...
    const cardContent = [
      ...
      '',
      '---',
      `**${localizeText('Reply', '回复')}**`,
      localizeText('`1` - Allow once', '`1` - 允许一次'),
      localizeText('`2` - Allow session', '`2` - 允许本会话'),
      localizeText('`3` - Deny', '`3` - 拒绝'),
      '',
      localizeText('Or use full commands:', '或使用完整命令：'),
      ...permCommands,
    ].join('\n');
    ...
    const localizedCardJson = cardJson
      .replace('馃攼 Permission Required', localizeText('Permission Required', '需要权限确认'));

    const fallbackCardJson = JSON.stringify({
      schema: '2.0',
      config: { wide_screen_mode: true },
      header: {
        template: 'orange',
    ...
    try {
      const res = await this.restClient.im.message.create({
        ...
        data: {
          receive_id: chatId,
          msg_type: 'interactive',
          content: fallbackCardJson || localizedCardJson,
        },
      });
      if (res?.data?.message_id) {
        return { ok: true, messageId: res.data.message_id };
    ...
    const plainText = [
      ...
      '',
      '---',
      localizeText('Reply: 1 = Allow once | 2 = Allow session | 3 = Deny', '回复：1 = 允许一次 | 2 = 允许本会话 | 3 = 拒绝'),
      '',
      ...permCommands,
    ].join('\n');
    ...
    const localizedPlainText = [
      mdText,
      '',
      '---',
      localizeText(
        'Reply: 1 = Allow once | 2 = Allow session | 3 = Deny',
        '\u56DE\u590D\uFF1A1 = \u5141\u8BB8\u4E00\u6B21 | 2 = \u5141\u8BB8\u672C\u4F1A\u8BDD | 3 = \u62D2\u7EDD',
      ),
      '',
      ...permCommands,
    ].join('\n');
    ...
    try {
      const res = await this.restClient.im.message.create({
        ...
        data: {
          receive_id: chatId,
          msg_type: 'text',
          content: JSON.stringify({ text: localizedPlainText || fallbackPlainText || plainText }),
        },
      });
      if (res?.data?.message_id) {
        return { ok: true, messageId: res.data.message_id };
  ...
  private addCardActionToDedup(token: string): void {
    this.seenCardActionTokens.set(token, true);

    if (this.seenCardActionTokens.size > DEDUP_MAX) {
      const excess = this.seenCardActionTokens.size - DEDUP_MAX;
      let removed = 0;
      for (const key of this.seenCardActionTokens.keys()) {
        if (removed >= excess) break;
        this.seenCardActionTokens.delete(key);
        removed++;
      }
    }
  }
