...
export function resolve(address: ChannelAddress): ChannelBinding {
  ...
  const existing = store.getChannelBinding(address.channelType, address.chatId, getAddressBindingKey(address));
  if (existing) {
...
export function bindToSession(
  ...
  const sdkSessionId = typeof (session as unknown as { sdk_session_id?: unknown }).sdk_session_id === 'string'
    ? (session as unknown as { sdk_session_id: string }).sdk_session_id
    : '';
