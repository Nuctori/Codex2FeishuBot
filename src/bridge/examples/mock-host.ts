...
class InMemoryStore implements BridgeStore {
  ...
  createSession(name: string, model: string, _sp?: string, cwd?: string, _mode?: string, timestamps?: SessionTimestamps) {
  ...
  updateSessionMetadata(sessionId: string, updates: Partial<Pick<BridgeSession, 'name' | 'working_directory' | 'model' | 'created_at' | 'updated_at'>>) {
    const session = this.sessions.get(sessionId);
    if (!session) return;
